...
import { PoModalComponent, PoButtonComponent, PoCheckboxComponent, PoSwitchComponent } from '@po-ui/ng-components';
...
export class ThemeBuilderComponent implements AfterViewInit {
  ...
  private checkChangesButtonD(changes: { [key: string]: string }): void {
    if (!this.isEmpty(changes)) {
      this.resultButtonD['nativeElement'].innerHTML = '/*button Secondary*/ <br>po-button {<br>';
  ...
  private checkChangesButtonL(changes: { [key: string]: string }): void {
    if (!this.isEmpty(changes)) {
      this.resultButtonL['nativeElement'].innerHTML = '/*button Tertiary*/ <br>po-button {<br>';
